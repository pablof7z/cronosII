...
c2_address_book_card_get (guint offset) {
  ...
  if (node) card = C2VCARD (node->data);
...
c2_address_book_card_add (const C2VCard *card) {
  ...
  addrbook = g_list_insert_sorted (addrbook, (gpointer) card, (GCompareFunc) sort_cards_by_name_show);
  return g_list_position (addrbook, g_list_find (addrbook, (gpointer) card));
}
...
c2_address_book_flush (void) {
  ...
  for (list = addrbook; list != NULL; list = list->next) {
	...
	g_string_append (buf, ";INTL");
      ...
      g_string_append (buf, ":");
      if (addr->post_office) g_string_append (buf, addr->post_office);
      g_string_append (buf, ";");
      if (addr->extended) g_string_append (buf, addr->extended);
      g_string_append (buf, ";");
      if (addr->street) g_string_append (buf, addr->street);
      g_string_append (buf, ";");
      if (addr->city) g_string_append (buf, addr->city);
      g_string_append (buf, ";");
      if (addr->region) g_string_append (buf, addr->region);
      g_string_append (buf, ";");
      if (addr->postal_code) g_string_append (buf, addr->postal_code);
      g_string_append (buf, ";");
      if (addr->country) g_string_append (buf, addr->country);
      fprintf (fd, "ADR%s\r\n", buf->str);
	...
	g_string_append (buf, ";VIDEO");
      ...
      g_string_append (buf, ":");
      if (phone->number) g_string_append (buf, phone->number);
      fprintf (fd, "TEL%s\r\n", buf->str);
      g_string_free (buf, TRUE);
    }
	...
	g_string_append (buf, ";X400");
      ...
      g_string_append (buf, ":");
      if (email->address) g_string_append (buf, email->address);
      fprintf (fd, "EMAIL%s\r\n", buf->str);
      g_string_free (buf, TRUE);
    }
    ...
    if (card->org_title) fprintf (fd, "TITLE:%s\r\n", card->org_title);
    if (card->org_name) fprintf (fd, "ORG:%s\r\n", card->org_name);
    if (card->web) fprintf (fd, "URL:%s\r\n", card->web);
    if (card->birthday) fprintf (fd, "BDAY:%s\r\n", card->birthday);
    if (card->categories) {
      unsigned char *ptr;
	...
	if (*ptr < 128 && *ptr != '\n') g_string_append_c (buf, *ptr);
	  ...
	  char *lbuf = g_strdup_printf ("=%2X", *ptr);
	...
	if (*ptr < 128 && *ptr != '\n') g_string_append_c (buf, *ptr);
	  ...
	  char *lbuf = g_strdup_printf ("=%02X", *ptr);
	  g_string_append (buf, lbuf);
	  c2_free (lbuf);
	}
      }
	...
	if (!(line = str_get_line (ptr))) break;
	size = strlen (line);
	ptr += size;
	line[size-1] = '\r';
	fprintf (fd, "%s\n", line);
 ...
 **/
int
c2_address_book_search (GList *search_list, const char *pattern, guint offset) {
  int i;
  char *mpattern = NULL;
  GList *list, *li;
  C2VCard *card;
  ...
  for (list = g_list_nth (search_list, offset), i = offset; list != NULL; list = list->next, i++) {
    card = list->data;
    if (!card) continue;
    
    /* Names */
    if ((card->name_show) &&
	(!fnmatch (mpattern, card->name_show, 1 << 4))) goto end_search;
    if ((card->name_salutation) &&
	(!fnmatch (mpattern, card->name_salutation, 1 << 4))) goto end_search;
    if ((card->name_first) &&
	(!fnmatch (mpattern, card->name_first, 1 << 4))) goto end_search;
    if ((card->name_middle) &&
	(!fnmatch (mpattern, card->name_middle, 1 << 4))) goto end_search;
    if ((card->name_last) &&
	(!fnmatch (mpattern, card->name_last, 1 << 4))) goto end_search;
    if ((card->name_suffix) &&
	(!fnmatch (mpattern, card->name_suffix, 1 << 4))) goto end_search;

    /* Birthday */
    if ((card->birthday) &&
	(!fnmatch (mpattern, card->birthday, 1 << 4))) goto end_search;

    /* Organization */
    if ((card->org_name) &&
	(!fnmatch (mpattern, card->org_name, 1 << 4))) goto end_search;
    if ((card->org_title) &&
	(!fnmatch (mpattern, card->org_title, 1 << 4))) goto end_search;

    /* Groups */
    for (li = card->groups; li != NULL; li = li->next)
      if (!fnmatch (mpattern, li->data, 1 << 4)) goto end_search;

    /* Web */
    if ((card->web) &&
        (!fnmatch (mpattern, card->web, 1 << 4))) goto end_search;

    /* Email */
    for (li = card->email; li != NULL; li = li->next)
      if ((C2VCARDEMAIL (li->data)->address) &&
	  (!fnmatch (mpattern, C2VCARDEMAIL (li->data)->address, 1 << 4))) goto end_search;

    /* Address */
    for (li = card->address; li != NULL; li = li->next) {
      if ((C2VCARDADDRESS (li->data)->post_office) &&
	  (!fnmatch (mpattern, C2VCARDADDRESS (li->data)->post_office, 1 << 4))) goto end_search;
      if ((C2VCARDADDRESS (li->data)->extended) &&
	  (!fnmatch (mpattern, C2VCARDADDRESS (li->data)->extended, 1 << 4))) goto end_search;
      if ((C2VCARDADDRESS (li->data)->street) &&
	  (!fnmatch (mpattern, C2VCARDADDRESS (li->data)->street, 1 << 4))) goto end_search;
      if ((C2VCARDADDRESS (li->data)->city) &&
	  (!fnmatch (mpattern, C2VCARDADDRESS (li->data)->city, 1 << 4))) goto end_search;
      if ((C2VCARDADDRESS (li->data)->region) &&
	  (!fnmatch (mpattern, C2VCARDADDRESS (li->data)->region, 1 << 4))) goto end_search;
      if ((C2VCARDADDRESS (li->data)->postal_code) &&
	  (!fnmatch (mpattern, C2VCARDADDRESS (li->data)->postal_code, 1 << 4))) goto end_search;
      if ((C2VCARDADDRESS (li->data)->country) &&
	  (!fnmatch (mpattern, C2VCARDADDRESS (li->data)->country, 1 << 4))) goto end_search;
    }

    /* Phone */
    for (li = card->phone; li != NULL; li = li->next)
      if ((C2VCARDPHONE (li->data)->number) &&
	  (!fnmatch (mpattern, C2VCARDPHONE (li->data)->number, 1 << 4))) goto end_search;

    /* Categories */
    if ((card->categories) &&
	(!fnmatch (mpattern, card->categories, 1 << 4))) goto end_search;

    /* Comment */
    if ((card->comment) &&
	(!fnmatch (mpattern, card->comment, 1 << 4))) goto end_search;

    /* Security */
    if ((card->security) &&
	(!fnmatch (mpattern, card->security, 1 << 4))) goto end_search;
...
c2_address_book_search_all (const char *pattern) {
  ...
  for (list = addrbook; list != NULL; list = list->next) {
    ...
    /* Email */
    for (li = card->email; li != NULL; li = li->next)
      if (!fnmatch (mpattern, C2VCARDEMAIL (li->data)->address, 0)) {
	ret = g_list_append (ret, card);
	goto move_on;
      }
    ...
    /* Address */
    for (li = card->address; li != NULL; li = li->next) {
      if (!fnmatch (mpattern, C2VCARDADDRESS (li->data)->post_office, 0)) {
	ret = g_list_append (ret, card);
	goto move_on;
      }
      if (!fnmatch (mpattern, C2VCARDADDRESS (li->data)->extended, 0)) {
	ret = g_list_append (ret, card);
	goto move_on;
      }
      if (!fnmatch (mpattern, C2VCARDADDRESS (li->data)->street, 0)) {
	ret = g_list_append (ret, card);
	goto move_on;
      }
      if (!fnmatch (mpattern, C2VCARDADDRESS (li->data)->city, 0)) {
	ret = g_list_append (ret, card);
	goto move_on;
      }
      if (!fnmatch (mpattern, C2VCARDADDRESS (li->data)->region, 0)) {
	ret = g_list_append (ret, card);
	goto move_on;
      }
      if (!fnmatch (mpattern, C2VCARDADDRESS (li->data)->postal_code, 0)) {
	ret = g_list_append (ret, card);
	goto move_on;
      }
      if (!fnmatch (mpattern, C2VCARDADDRESS (li->data)->country, 0)) {
	ret = g_list_append (ret, card);
	goto move_on;
      }
    }
    ...
    /* Phone */
    for (li = card->phone; li != NULL; li = li->next)
      if (!fnmatch (mpattern, C2VCARDPHONE (li->data)->number, 0)) {
      ret = g_list_append (ret, card);
      goto move_on;
    }
 ...
 **/
gboolean
c2_address_book_check_name_exists(const char *name) {
  GList *list;
  C2VCard *card;
  ...
  for (list = addrbook; list != NULL; list = list->next) {
    card = C2VCARD (list->data);
    if (streq (card->name_show, name)) return TRUE;
  }
...
c2_address_book_get_available_groups (void) {
  ...
  for (li = addrbook; li != NULL; li = li->next) {
	...
	if (streq (CHAR (s->data), CHAR (le->data))) goto skip;
      }
...
c2_address_book_init (void) {
  ...
  if ((fd = fopen (path, "r")) == NULL) {
    if (errno != ENOENT) {
      gdk_threads_enter ();
      cronos_error (errno, _("Couldn't open the GnomeCard file"), ERROR_WARNING);
      gdk_threads_leave ();
    }
    c2_free(path);
    return;
  ...
  for (;;) {
    /* Get the card */
    if ((card = c2_address_book_get_card_from_fd (fd)) == NULL) break;
...
c2_address_book_get_card_from_str (const char *string) {
  ...
  for (ptr = string;;) {
    if ((line = str_get_line (ptr)) == NULL) return NULL;
    if (strneq (line, "BEGIN:VCARD", 11)) break;
    c2_free (line);
  }
  c2_free (line);
  ...
  for (;;) {
    if ((line = str_get_line (ptr)) == NULL) break;
    ptr += strlen (line);
    ...
    if ((strneq (line, "FN", 2)) &&
	(line[2] == separator == ';' ? ';' : ':')) {
      card->name_show = g_strndup (line+offset+1, strlen (line)-4);
    }
    else if ((strneq (line, "TITLE", 5)) &&
	     (line[5] == separator == ';' ? ';' : ':')) {
      card->org_title = g_strndup (line+offset+1, strlen (line)-7);
    }
    else if ((strneq (line, "ORG", 3)) &&
	     (line[3] == separator == ';' ? ';' : ':')) {
      card->org_name = g_strndup (line+offset+1, strlen (line)-5);
    }
    else if ((strneq (line, "CATEGORIES", 10)) &&
	     (line[10] == separator == ';' ? ';' : ':')) {
      card->categories = g_strndup (line+offset+1, strlen (line)-12);
    }
    else if ((strneq (line, "NOTE", 4)) &&
	     (line[4] == separator == ';' ? ';' : ':')) {
      card->comment = g_strndup (line+offset+1, strlen (line)-6);
    }
    else if ((strneq (line, "URL", 3)) &&
	     (line[3] == separator == ';' ? ';' : ':')) {
      card->web = g_strndup (line+offset+1, strlen (line)-5);
    }
    else if ((strneq (line, "BDAY", 4)) &&
	     (line[4] == separator == ';' ? ';' : ':')) {
      card->birthday = g_strndup (line+offset+1, strlen (line)-6);
    }
    else if ((strneq (line, "KEY", 3)) &&
	     (line[3] == separator == ';' ? ';' : ':')) {
      card->security = c2_address_book_get_line_field_n (line+offset+1, separator, 1);
    }
    else if ((strneq (line, "N", 1)) &&
   	     (line[1] == separator == ';' ? ';' : ':')) {
      card->name_last = c2_address_book_get_line_field_n (line+offset+1, separator, 0);
      card->name_first = c2_address_book_get_line_field_n (line+offset+1, separator, 1);
      card->name_middle = c2_address_book_get_line_field_n (line+offset+1, separator, 2);
      card->name_salutation = c2_address_book_get_line_field_n (line+offset+1, separator, 3);
      card->name_suffix = c2_address_book_get_line_field_n (line+offset+1, separator, 4);
    }
      ...
      for (i = 0;; i++) {
	if ((buf = c2_address_book_get_line_field_n (line+offset+1, separator, i)) == NULL) break;

	card->groups = g_list_append (card->groups, buf);
      }
    }
    else if ((strneq (line, "ADR", 3)) &&
   	     (line[3] == separator == ';' ? ';' : ':')) {
      C2VCardAddress *addr = c2_address_book_card_address_new ();
      char *types, *spec, *buf;
      char mysep;
      int i;
      ...
      types = c2_address_book_get_line_field_n (line+offset+1, separator, 0);
      spec = c2_address_book_get_line_field_n (line+offset+1, separator, 1);

      /* Learn the types to the object */
      for (i = 0;; i++) {
	if ((buf = c2_address_book_get_line_field_n (types, mysep, i)) == NULL) break;

	if (streq (buf, "DOM")) addr->type ^= 1 << 5;
	else if (streq (buf, "INTL")) addr->type ^= 1 << 6;
	else if (streq (buf, "POSTAL")) addr->type ^= 1 << 3;
	else if (streq (buf, "PARCEL")) addr->type ^= 1 << 4;
	else if (streq (buf, "HOME")) addr->type ^= 1 << 1;
	else if (streq (buf, "WORK")) addr->type ^= 1 << 2;
	c2_free (buf);
      }
      c2_free (types);

      /* Learn the address */
      addr->post_office = c2_address_book_get_line_field_n (spec, mysep, 0);
      addr->extended = c2_address_book_get_line_field_n (spec, mysep, 1);
      addr->street = c2_address_book_get_line_field_n (spec, mysep, 2);
      addr->city = c2_address_book_get_line_field_n (spec, mysep, 3);
      addr->region = c2_address_book_get_line_field_n (spec, mysep, 4);
      addr->postal_code = c2_address_book_get_line_field_n (spec, mysep, 5);
      addr->country = c2_address_book_get_line_field_n (spec, mysep, 6);
      ...
      /* Get the types and spec of address */
      types = c2_address_book_get_line_field_n (line+offset+1, separator, 0);
      ph->number = c2_address_book_get_line_field_n (line+offset+1, separator, 1);

      /* Learn the types to the object */
      for (i = 0;; i++) {
	if ((buf = c2_address_book_get_line_field_n (types, mysep, i)) == NULL) break;

	if (streq (buf, "PREF")) ph->type ^= 1 << 1;
	else if (streq (buf, "WORK")) ph->type ^= 1 << 2;
	else if (streq (buf, "HOME")) ph->type ^= 1 << 3;
	else if (streq (buf, "VOICE")) ph->type ^= 1 << 4;
	else if (streq (buf, "FAX")) ph->type ^= 1 << 5;
	else if (streq (buf, "MSG")) ph->type ^= 1 << 6;
	else if (streq (buf, "CELL")) ph->type ^= 1 << 7;
	else if (streq (buf, "PAGER")) ph->type ^= 1 << 8;
	else if (streq (buf, "BBS")) ph->type ^= 1 << 9;
	else if (streq (buf, "MODEM")) ph->type ^= 1 << 10;
	else if (streq (buf, "CAR")) ph->type ^= 1 << 11;
	else if (streq (buf, "ISDN")) ph->type ^= 1 << 12;
	else if (streq (buf, "VIDEO")) ph->type ^= 1 << 13;
	c2_free (buf);
      }
      c2_free (types);
      ...
      /* Get the types and spec of address */
      types = c2_address_book_get_line_field_n (line+offset+1, separator, 0);
      em->address = c2_address_book_get_line_field_n (line+offset+1, separator, 1);

      /* Learn the types to the object */
      for (i = 0;; i++) {
	if ((buf = c2_address_book_get_line_field_n (types, mysep, i)) == NULL) break;

	if (streq (buf, "AOL")) em->type ^= 1 << 1;
	else if (streq (buf, "APPLELINK")) em->type ^= 1 << 2;
	else if (streq (buf, "ATTMAIL")) em->type ^= 1 << 3;
	else if (streq (buf, "CIS")) em->type ^= 1 << 4;
	else if (streq (buf, "EWORLD")) em->type ^= 1 << 5;
	else if (streq (buf, "INTERNET")) em->type ^= 1 << 6;
	else if (streq (buf, "IBMMail")) em->type ^= 1 << 7;
	else if (streq (buf, "MCIMAIL")) em->type ^= 1 << 8;
	else if (streq (buf, "POWERSHARE")) em->type ^= 1 << 9;
	else if (streq (buf, "PRODIGY")) em->type ^= 1 << 10;
	else if (streq (buf, "TLX")) em->type ^= 1 << 11;
	else if (streq (buf, "X400")) em->type ^= 1 << 12;
	c2_free (buf);
	break;
      }
      c2_free (types);
      ...
      if (!other) other = g_string_new (NULL);
...
c2_address_book_get_card_from_fd (FILE *fd) {
  ...
  /* Find the start of the card */
  for (;;) {
    if ((line = c2_address_book_get_line (fd)) == NULL) return NULL;
    if (strneq (line, "BEGIN:VCARD", 11)) break;
    c2_free (line);
  }
  c2_free (line);
  ...
  /* Start getting all the fields */
  for (;;) {
    if ((line = c2_address_book_get_line (fd)) == NULL) break;
...
c2_address_book_get_line (FILE *fd) {
  ...
  for (i=0;;) {
    if (i) g_string_append_c (abline, separator);
	...
	if (key[size-1] == '\r') key[size-1] = '\n';
      ...
      decode = decode_quoted_printable (qpbuf->str, &qpbuf->len);
 ...
 **/
gchar *cut_leading_chars(gchar * str, gint num) {
  gint i;
  gchar *new_str;
 ...
 * Returns: the new str
 */
gchar *cut_leading_spaces(gchar * str) {
  gint counter;
