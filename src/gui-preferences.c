...
gui_preferences_new (void)
{
  ...
  gtk_signal_connect (GTK_OBJECT (sections_ctree), "select_row",
		      GTK_SIGNAL_FUNC (on_prefs_sections_clist_select_row),
		      NULL);

  label[0] = g_strdup (_("General"));
  label[1] = "";
  ctree_general =
    gtk_ctree_insert_node (GTK_CTREE (sections_ctree), NULL, NULL, label,
  ...
  gtk_box_pack_start (GTK_BOX (sections_vbox), browse_prev_btn, FALSE, TRUE,
		      0);
  ...
  gtk_box_pack_start (GTK_BOX (sections_vbox), browse_next_btn, FALSE, TRUE,
		      0);
...
#  define PAGES_IN_NOTEBOOK 8
  ...
  gnome_dialog_button_connect (GNOME_DIALOG (window), 2,
			       on_prefs_cancel_btn_clicked, NULL);
  gnome_dialog_button_connect (GNOME_DIALOG (window), 1,
			       on_prefs_ok_btn_clicked, NULL);
  gnome_dialog_button_connect (GNOME_DIALOG (window), 0,
			       on_prefs_ok_btn_clicked, (gpointer) TRUE);

  gtk_widget_show (window);
}

static void on_prefs_ok_btn_clicked_write_mailboxes (Mailbox * head,
						     FILE * fd);
static void on_prefs_ok_btn_clicked_write_accounts (Account * head,
						    FILE * fd);

static void
on_prefs_ok_btn_clicked (GtkWidget * widget, gpointer accepting)
{
  GtkWidget *menu;
  char *buf;
  FILE *fd;
  ...
  if ((fd = fopen (buf, "w")) == NULL)
    {
      cronos_error (errno, "Opening config file to write configuration",
		    ERROR_WARNING);
      c2_free (buf);
      return;
    }
  ...
  create_mailbox_tree (WMain->window, WMain->ctree, NULL,
		       config->mailbox_head);
  ...
  if (config->check_timeout != (int) options_mail_check->value)
	...
	check_timeout =
	  gtk_timeout_add ((config->check_timeout * 60) * 1000,
			   (GtkFunction) check_timeout_check, NULL);
    }
  fprintf (fd, "\n\n"
  ...
  config->check_at_start =
    GTK_TOGGLE_BUTTON (options_check_at_start)->active ? TRUE : FALSE;
  fprintf (fd, "\n" "* Check at start\n" "check_at_start %d\n",
	   config->check_at_start);
  ...
  config->empty_garbage =
    GTK_TOGGLE_BUTTON (options_empty_garbage)->active ? 1 : 0;
  fprintf (fd,
  ...
  config->prepend_char_on_re =
    g_strdup (gtk_entry_get_text
	      (GTK_ENTRY (options_prepend_character_entry)));
  if (!config->prepend_char_on_re)
   ...
   *********************************/
  config->timeout =
    gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON (advanced_timeout));
  fprintf (fd, "\n" "* Timeout for net related stuff\n" "timeout %d\n",
	   config->timeout);

  /** Persistent SMTP Connection **/
  config->use_persistent_smtp_connection =
    GTK_TOGGLE_BUTTON (advanced_use_persistent_smtp_connection)->active ? 1 :
    0;
  fprintf (fd,
  ...
  if (streq (buf, _("when required.")))
    config->addrbook_init = INIT_ADDRESS_BOOK_INIT_REQ;
  else if (streq (buf, _("when it is opened.")))
    config->addrbook_init = INIT_ADDRESS_BOOK_INIT_OPEN;
  else
  ...
  /***************************************
   * Do not Download message larger than *
   ***************************************/
  config->message_bigger =
    gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON
				      (options_message_bigger));
  fprintf (fd,
   ...
   * COLORS *
   **********/
  gnome_color_picker_get_i16 (GNOME_COLOR_PICKER
			      (appareance_colors_reply_original_message),
  ...
  gdk_color_alloc (gdk_colormap_get_system (),
		   &config->color_reply_original_message);
  ...
  if (GTK_TOGGLE_BUTTON (appareance_toolbar_both)->active)
    rc->toolbar = GTK_TOOLBAR_BOTH;
  else if (GTK_TOGGLE_BUTTON (appareance_toolbar_xpm)->active)
    rc->toolbar = GTK_TOOLBAR_ICONS;
  else if (GTK_TOGGLE_BUTTON (appareance_toolbar_text)->active)
    rc->toolbar = GTK_TOOLBAR_TEXT;
  ...
  rc->font_body =
    g_strdup (gtk_entry_get_text (GTK_ENTRY (appareance_font_message_body)));
  rc->font_unread =
    g_strdup (gtk_entry_get_text
	      (GTK_ENTRY (appareance_font_unread_message_list)));
  rc->font_read =
    g_strdup (gtk_entry_get_text
	      (GTK_ENTRY (appareance_font_read_message_list)));
  rc->font_print =
    g_strdup (gtk_entry_get_text (GTK_ENTRY (appareance_font_print)));
...
static void
on_prefs_ok_btn_clicked_write_mailboxes (Mailbox * head, FILE * fd)
{
  Mailbox *mailbox;
  char *buf;
  DIR *dir;
  ...
  for (mailbox = head; mailbox; mailbox = mailbox->next)
    {
      fprintf (fd, "mailbox \"%d\r%s\r%d\"\n", mailbox->self_id,
	       mailbox->name, mailbox->parent_id);
      if (mailbox->child != NULL)
	on_prefs_ok_btn_clicked_write_mailboxes (mailbox->child, fd);

      /* Check that the directory exists */
      buf =
	g_strconcat (getenv ("HOME"), ROOT, "/", mailbox->name, ".mbx", NULL);
	  ...
	  if (errno != ENOENT)
	    {
	      cronos_error (errno,
			    _
			    ("Checking if the  mailbox directory exists"),
			    ERROR_WARNING);
	      c2_free (buf);
	      continue;
	    }
      ...
      else
	{
      ...
      c2_free (buf);
      buf =
	g_strconcat (getenv ("HOME"), ROOT, "/", mailbox->name,
		     ".mbx/index", NULL);
      if ((file = fopen (buf, "w")) == NULL)
	{
	  cronos_error (errno,
			_
			("Creating the index directory for the new mailbox"),
			ERROR_WARNING);
	  c2_free (buf);
	  continue;
	}
...
static void
on_prefs_ok_btn_clicked_write_accounts (Account * head, FILE * fd)
{
  Account *account;
  char *buf;
  ...
  for (account = head; account; account = account->next)
	...
	fprintf (fd, "%s\r%s\r%s\r%d\r%d\r%s\"\n",
		 account->protocol.pop.usr_name,
		 account->protocol.pop.pass, account->protocol.pop.host,
		 account->protocol.pop.host_port,
		 account->always_append_signature, account->signature);
      else if (account->type == C2_ACCOUNT_SPOOL)
...
static void
on_prefs_sections_clist_select_row (GtkWidget * widget, gint row)
{
  int page;
  ...
  else if (page == PAGES_IN_NOTEBOOK)
    {
...
static void
on_prefs_previous_btn_clicked (void)
{
  int current_page;
  ...
  else if (current_page == PAGES_IN_NOTEBOOK)
    {
      gtk_widget_set_sensitive (browse_prev_btn, TRUE);
      gtk_widget_set_sensitive (browse_next_btn, FALSE);
    }
...
static void
on_prefs_next_btn_clicked (void)
{
  int current_page;
...
static void
on_prefs_cancel_btn_clicked (void)
{
  gtk_widget_destroy (window);
}

static void
gui_destroy_data (GtkWidget * widget, gpointer data)
{
...
static void
on_account_new_ok_clicked (GtkWidget * widget, gpointer data)
{
  ...
  if (!mbox_name)
    {
      g_warning (_
		 ("Please select a mailbox  to store messages Downloaded from this account."));
      return;
    }
  ...
  /* Load the data */
  account->acc_name =
    g_strdup (gtk_entry_get_text (GTK_ENTRY (_account->acc_name)));
  account->per_name =
    g_strdup (gtk_entry_get_text (GTK_ENTRY (_account->per_name)));
  account->mail_addr =
    g_strdup (gtk_entry_get_text (GTK_ENTRY (_account->mail_addr)));
  ...
  if (streq (new_account_type, "POP"))
    {
      ...
      account->protocol.pop.usr_name =
	g_strdup (gtk_entry_get_text
		  (GTK_ENTRY (_account->protocol.pop.usr_name)));
      account->protocol.pop.pass =
	g_strdup (gtk_entry_get_text
		  (GTK_ENTRY (_account->protocol.pop.pass)));
      account->protocol.pop.host =
	g_strdup (gtk_entry_get_text
		  (GTK_ENTRY (_account->protocol.pop.host)));
      account->protocol.pop.host_port =
	gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON
					  (_account->protocol.pop.host_port));
    }
  else if (streq (new_account_type, "SPOOL"))
    {
      ...
      account->protocol.spool.file =
	g_strdup (gtk_entry_get_text
		  (GTK_ENTRY (_account->protocol.spool.file)));
    }
  account->smtp = g_strdup (gtk_entry_get_text (GTK_ENTRY (_account->smtp)));
  account->smtp_port =
    gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON (_account->smtp_port));
  if (GTK_TOGGLE_BUTTON (_account->keep_copy)->active)
    account->keep_copy = TRUE;
  else
    account->keep_copy = FALSE;
  if (GTK_TOGGLE_BUTTON (_account->use_it)->active)
    account->use_it = TRUE;
  else
    account->use_it = FALSE;
  account->always_append_signature =
    GTK_TOGGLE_BUTTON (_account->
		       always_append_signature)->active ? TRUE : FALSE;
  account->signature =
    g_strdup (gtk_editable_get_chars
	      (GTK_EDITABLE (_account->signature), 0, -1));
...
static void
on_account_edit_ok_clicked (GtkWidget * widget, gpointer data)
{
  PreferencesAccount *_account;
  Account *account;
  Account *acc_ll;
  ...
  c2_free (acc_ll->acc_name);
  acc_ll->acc_name = account->acc_name;
  c2_free (acc_ll->per_name);
  acc_ll->per_name = account->per_name;
  c2_free (acc_ll->mail_addr);
  acc_ll->mail_addr = account->mail_addr;
  if (account->type == C2_ACCOUNT_POP)
    {
      c2_free (acc_ll->protocol.pop.usr_name);
      acc_ll->protocol.pop.usr_name = account->protocol.pop.usr_name;
      c2_free (acc_ll->protocol.pop.pass);
      acc_ll->protocol.pop.pass = account->protocol.pop.pass;
      c2_free (acc_ll->protocol.pop.host);
      acc_ll->protocol.pop.host = account->protocol.pop.host;
      acc_ll->protocol.pop.host_port = account->protocol.pop.host_port;
    }
  else if (account->type == C2_ACCOUNT_SPOOL)
    {
      c2_free (acc_ll->protocol.spool.file);
      acc_ll->protocol.spool.file = account->protocol.spool.file;
    }
  c2_free (acc_ll->smtp);
  acc_ll->smtp = account->smtp;
  ...
  c2_free (acc_ll->signature);
  acc_ll->signature = account->signature;
...
static void
account_new_or_edit_mailbox_select_row (GtkCTree * ctree, gint row, gint col,
					GdkEvent * event, gpointer data)
{
  GtkCTreeNode *node;
  node = gtk_ctree_node_nth (ctree, row);
  mbox_name =
    ((Mailbox *) gtk_ctree_node_get_row_data (GTK_CTREE (ctree), node))->name;
}

static void
on_account_new_or_edit_mail_addr_changed (GtkWidget * widget, gpointer acc)
{
  ...
  if (account->type == C2_ACCOUNT_SPOOL)
    return;
  ...
  if (has_at)
    gtk_entry_set_text (GTK_ENTRY (account->protocol.pop.host), host);
...
static void
on_account_new_or_edit_select_file_btn_clicked (GtkWidget * btn,
						GtkWidget * entry)
{
...
static char *
account_new_or_edit (Account * _account)
{
  GtkWidget *vbox;
  GtkWidget *table;
  GtkWidget *hsep;
  ...
  if (!_account)
    {
      if (streq (new_account_type, "SPOOL"))
	account->type = C2_ACCOUNT_SPOOL;
      else if (streq (new_account_type, "POP"))
	account->type = C2_ACCOUNT_POP;
      else
	account->type = C2_ACCOUNT_POP;
    }
  ...
  gtk_window_set_title (GTK_WINDOW (account->window),
			_account ? _("Edit Account") : _("New Account"));
  gtk_window_set_modal (GTK_WINDOW (account->window), TRUE);
  gtk_window_set_transient_for (GTK_WINDOW (account->window),
				GTK_WINDOW (window));
  ...
  gtk_table_attach_defaults (GTK_TABLE (table), account->acc_name, 1, 2, 0,
			     1);
  ...
  gtk_table_attach_defaults (GTK_TABLE (table), account->per_name, 1, 2, 1,
			     2);
  gtk_widget_show (account->per_name);
  if (_account != NULL)
    gtk_entry_set_text (GTK_ENTRY (account->per_name), _account->per_name);

  account->mail_addr = gtk_entry_new ();
  gtk_table_attach_defaults (GTK_TABLE (table), account->mail_addr, 1, 2, 2,
			     3);
  gtk_widget_show (account->mail_addr);
  if (_account != NULL)
    gtk_entry_set_text (GTK_ENTRY (account->mail_addr), _account->mail_addr);
  gtk_signal_connect (GTK_OBJECT (account->mail_addr), "changed",
		      GTK_SIGNAL_FUNC
		      (on_account_new_or_edit_mail_addr_changed), account);
  ...
  if (account->type == C2_ACCOUNT_POP)
    {
      ...
      gtk_table_attach_defaults (GTK_TABLE (table),
				 account->protocol.pop.usr_name, 1, 2, 0, 1);
      gtk_widget_show (account->protocol.pop.usr_name);
      if (_account != NULL)
	gtk_entry_set_text (GTK_ENTRY (account->protocol.pop.usr_name),
			    _account->protocol.pop.usr_name);

      account->protocol.pop.pass = gtk_entry_new ();
      gtk_table_attach_defaults (GTK_TABLE (table),
				 account->protocol.pop.pass, 1, 2, 1, 2);
      gtk_widget_show (account->protocol.pop.pass);
      gtk_entry_set_visibility (GTK_ENTRY (account->protocol.pop.pass),
				FALSE);
      if (_account != NULL)
	gtk_entry_set_text (GTK_ENTRY (account->protocol.pop.pass),
			    _account->protocol.pop.pass);
      ...
      gtk_box_pack_start (GTK_BOX (hbox), account->protocol.pop.host, TRUE,
			  TRUE, 0);
      gtk_widget_show (account->protocol.pop.host);
      if (_account != NULL)
	gtk_entry_set_text (GTK_ENTRY (account->protocol.pop.host),
			    _account->protocol.pop.host);
      ...
      adj =
	(GtkAdjustment *) gtk_adjustment_new ((_account != NULL) ?
					      _account->protocol.
					      pop.host_port : 110, 1, 99999,
					      1, 10, 0);
      account->protocol.pop.host_port = gtk_spin_button_new (adj, 1, 0);
      gtk_box_pack_start (GTK_BOX (hbox), account->protocol.pop.host_port,
			  TRUE, TRUE, 0);
      ...
      if (_account != NULL)
	gtk_entry_set_text (GTK_ENTRY (account->smtp), _account->smtp);
      if (config->use_persistent_smtp_connection)
	gtk_widget_set_sensitive (account->smtp, FALSE);
	...
	(GtkAdjustment *) gtk_adjustment_new ((_account != NULL) ?
      ...
      if (config->use_persistent_smtp_connection)
	gtk_widget_set_sensitive (account->smtp_port, FALSE);
    }
  else if (account->type == C2_ACCOUNT_SPOOL)
    {
      ...
      gtk_box_pack_start (GTK_BOX (hbox), account->protocol.spool.file,
			  TRUE, TRUE, 0);
      gtk_widget_show (account->protocol.spool.file);
      if (_account != NULL)
	gtk_entry_set_text (GTK_ENTRY (account->protocol.spool.file),
			    _account->protocol.spool.file);
      gtk_entry_set_editable (GTK_ENTRY (account->protocol.spool.file),
			      FALSE);
      ...
      gtk_signal_connect (GTK_OBJECT (btn), "clicked",
			  GTK_SIGNAL_FUNC
			  (on_account_new_or_edit_select_file_btn_clicked),
  ...
  gtk_signal_connect (GTK_OBJECT (account->mailbox), "select_row",
		      GTK_SIGNAL_FUNC
		      (account_new_or_edit_mailbox_select_row), NULL);
  if (_account)
    {
      ...
      for (i = 1, cnode =
	   gtk_ctree_node_nth (GTK_CTREE (account->mailbox), 0); cnode; i++)
	{
	  if (i > 1)
	    cnode = gtk_ctree_node_nth (GTK_CTREE (account->mailbox), i);
	  cnode_sele =
	    gtk_ctree_find_by_row_data (GTK_CTREE (account->mailbox),
					cnode, _account->mailbox);
...
out:
  ...
  if (_account && _account->signature)
    {
      gtk_text_insert (GTK_TEXT (account->signature), NULL, NULL, NULL,
		       _account->signature, -1);
    }
  else if (!_account)
    {
      FILE *fd;
...
doesnt_have_signature_file:
  ...
  account->always_append_signature =
    gtk_check_button_new_with_label (_("Automatically append signature"));
  gtk_box_pack_start (GTK_BOX (table), account->always_append_signature,
		      FALSE, FALSE, 1);
  gtk_widget_show (account->always_append_signature);
  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON
				(account->always_append_signature), FALSE);
  if (_account)
    {
  ...
  account->keep_copy =
    gtk_check_button_new_with_label (_("Keep messages on the server."));
  gtk_table_attach_defaults (GTK_TABLE (table), account->keep_copy, 0, 1, 0,
			     1);
  ...
  account->use_it =
    gtk_check_button_new_with_label (_("Activate this account"));
  ...
  if (_account != NULL)
    {
      if (!_account->use_it)
	{
	  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (account->use_it),
					FALSE);
	}
    }
  ...
  if (_account == NULL)
    {
      gtk_signal_connect (GTK_OBJECT (btn), "clicked",
			  GTK_SIGNAL_FUNC (on_account_new_ok_clicked),
			  account);
    }
  else
    {
      gtk_signal_connect (GTK_OBJECT (btn), "clicked",
			  GTK_SIGNAL_FUNC (on_account_edit_ok_clicked),
			  account);
    }
...
static void
on_account_ask_account_type_ok_btn_clicked (GtkWidget * widget,
...
static void
on_account_ask_account_type_cancel_btn_clicked (GtkWidget * widget,
...
static void
on_account_new_or_edit_btn_toggled (GtkWidget * btn,
				    PreferencesAccountType * actype)
{
  if (GTK_TOGGLE_BUTTON (actype->spool_btn)->active)
    {
      ...
      gtk_text_forward_delete (GTK_TEXT (actype->text),
			       gtk_text_get_length (GTK_TEXT (actype->text)));
...
account_ask_account_type (void)
{
  ...
  actype =
    (PreferencesAccountType *) g_malloc0 (sizeof (PreferencesAccountType));
  ...
  gtk_window_set_transient_for (GTK_WINDOW (actype->window),
				GTK_WINDOW (window));
  ...
  gtk_signal_connect (GTK_OBJECT (actype->pop_btn), "clicked",
		      GTK_SIGNAL_FUNC (on_account_new_or_edit_btn_toggled),
		      actype);

  actype->spool_btn =
    gtk_radio_button_new_with_label (grp, _("Spool Account"));
  ...
  gtk_signal_connect (GTK_OBJECT (btn), "clicked",
		      GTK_SIGNAL_FUNC
		      (on_account_ask_account_type_cancel_btn_clicked),
		      actype);
  ...
  gtk_signal_connect (GTK_OBJECT (btn), "clicked",
		      GTK_SIGNAL_FUNC
		      (on_account_ask_account_type_ok_btn_clicked), actype);
...
static void
on_account_clist_select_row_or_unselect_row (GtkWidget * clist)
{
...
#if FALSE			/* TODO */
      if (((int) GTK_CLIST (clist)->selection->data) ==
	  GTK_CLIST (clist)->rows - 1)
...
static void
on_account_edit (void)
{
  char *name;
  Account *account;

  if (!g_list_length (GTK_CLIST (account_clist)->selection))
    return;
  gtk_clist_get_text (GTK_CLIST (account_clist),
		      (gint) GTK_CLIST (account_clist)->selection->data, 0,
		      &name);
...
on_account_delete (void)
{
  ...
  for (iteration = 0, curr = account_list; curr;
  ...
  if (curr->acc_name)
    c2_free (curr->acc_name);
  if (curr->per_name)
    c2_free (curr->per_name);
  if (curr->type == C2_ACCOUNT_POP)
    {
      if (curr->protocol.pop.usr_name)
	c2_free (curr->protocol.pop.usr_name);
      if (curr->protocol.pop.pass)
	c2_free (curr->protocol.pop.pass);
      if (curr->protocol.pop.host)
	c2_free (curr->protocol.pop.host);
    }
  else if (curr->type == C2_ACCOUNT_SPOOL)
    {
      if (curr->protocol.spool.file)
	c2_free (curr->protocol.spool.file);
    }
  if (curr->smtp)
    c2_free (curr->smtp);
  c2_free (curr);
...
#if FALSE			/* TODO */
static void
on_account_up_btn_clicked (void)
{
  char *acc_name;
  Account *account;
  guint nth;
...
static void
on_account_down_btn_clicked (void)
{
  char *acc_name;
  Account *account;
  guint nth;
...
prefs_add_page_accounts (void)
{
  ...
  gtk_clist_set_column_title (GTK_CLIST (account_clist), 0,
			      _("Account Name"));
  ...
  gtk_signal_connect (GTK_OBJECT (account_clist), "select_row",
		      GTK_SIGNAL_FUNC
		      (on_account_clist_select_row_or_unselect_row), NULL);
  ...
  xpm =
    gnome_stock_pixmap_widget_at_size (WMain->window,
				       GNOME_STOCK_PIXMAP_NEW, 16, 16);
  ...
  xpm =
    gnome_stock_pixmap_widget_at_size (WMain->window,
				       GNOME_STOCK_PIXMAP_INDEX, 16, 16);
  ...
  xpm =
    gnome_stock_pixmap_widget_at_size (WMain->window,
				       GNOME_STOCK_PIXMAP_TRASH, 16, 16);
  ...
  gtk_widget_set_extension_events (account_down_btn,
				   GDK_EXTENSION_EVENTS_ALL);
  gtk_widget_add_accelerator (account_down_btn, "clicked", accel_group,
			      GDK_KP_1, 0, GTK_ACCEL_VISIBLE);
...
static void
on_mailboxes_tree_select_row (GtkCTree * ctree, gint row, gint col,
			      GdkEvent * event, gpointer data)
{
  GtkCTreeNode *node;
  Mailbox *mbox;
...
static void
on_mailboxes_tree_unselect_row (void)
{
  mailbox_selected_mbox = NULL;
}

static void
on_mailboxes_new_btn_clicked (void)
{
  Mailbox *parent = NULL;
  Mailbox *new;
  char *new_mbox_name;
  ...
  new_mbox_name =
    gtk_entry_get_text (GTK_ENTRY (mailboxes_new_mailbox_entry));
  if (!new_mbox_name)
    return;
  if (!strlen (new_mbox_name))
    return;
...
static void
on_mailboxes_delete_btn_clicked (void)
{
  Mailbox *search_mbox;
  Mailbox *working;
  Mailbox *parent;
  ...
  if (!strcmp (mailbox_selected_mbox, MAILBOX_INBOX) ||
    ...
    {
      g_warning (_
		 ("You can not delete the mailbox %s It is required for normal operation\n"),
mailbox_selected_mbox);
      return;
    }
...
prefs_add_page_mailboxes (void)
{
  ...
  gtk_signal_connect (GTK_OBJECT (mailboxes_tree), "select_row",
		      GTK_SIGNAL_FUNC (on_mailboxes_tree_select_row), NULL);
  gtk_signal_connect (GTK_OBJECT (mailboxes_tree), "unselect_row",
		      GTK_SIGNAL_FUNC (on_mailboxes_tree_unselect_row), NULL);
  create_mailbox_tree (window, mailboxes_tree, NULL, mailboxes_list);
  ...
  gtk_box_pack_start (GTK_BOX (hbox), mailboxes_new_mailbox_entry, TRUE,
		      TRUE, 0);
  ...
  gtk_widget_show (btn);
  gtk_signal_connect (GTK_OBJECT (btn), "clicked",
		      GTK_SIGNAL_FUNC (on_mailboxes_delete_btn_clicked),
		      NULL);
...
prefs_add_page_options (void)
{
  ...
  options_mail_check =
    GTK_ADJUSTMENT (gtk_adjustment_new
		    (config->check_timeout, 0, 100, 1, 0, 0));
   ...
   ** Mark as read **
   ******************/
  options_mark_as_read =
    GTK_ADJUSTMENT (gtk_adjustment_new
		    (config->mark_as_read, 0, 10, 1, 0, 0));
   ...
   ***********/
  label =
    gtk_label_new (_("String to prepend to the original mail contents: "));
  ...
  gtk_box_pack_start (GTK_BOX (hbox), options_prepend_character_entry,
		      FALSE, TRUE, 0);
  gtk_widget_show (options_prepend_character_entry);
  gtk_entry_set_text (GTK_ENTRY (options_prepend_character_entry),
		      config->prepend_char_on_re);
   ...
   ******************************************/
  adj =
    (GtkAdjustment *) gtk_adjustment_new (config->message_bigger, 0,
					  10000, 1, 100, 0);
  ...
  options_keep_copy =
    gtk_check_button_new_with_label (_
				     ("Store a copy of Sent mail in the Outbox "));
  gtk_box_pack_start (GTK_BOX (vbox), options_keep_copy, FALSE, FALSE, 0);
  gtk_widget_show (options_keep_copy);
  if (config->use_outbox)
    gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (options_keep_copy),
				  TRUE);
  gtk_tooltips_set_tip (tooltips, options_keep_copy,
			_
			("Selecting this option will have a copy of each mail sent, saved to the outbox."),
			NULL);
  /** Check for mail at start **/
  options_check_at_start =
    gtk_check_button_new_with_label (_("Check for mail at start up."));
  gtk_box_pack_start (GTK_BOX (vbox), options_check_at_start, FALSE, FALSE,
		      0);
  gtk_widget_show (options_check_at_start);
  if (config->check_at_start)
    gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON
				  (options_check_at_start), TRUE);
  gtk_tooltips_set_tip (tooltips, options_check_at_start,
			_
			("Check this to have Cronos II look for new mail at all configured Accounts on launch."),
			NULL);
 ...
 *******************************************************************/
static void
on_appareance_font_ok_btn_clicked (GtkWidget * btn, gpointer data)
{
  ...
  font =
    gtk_font_selection_get_font_name (GTK_FONT_SELECTION (widget->font_sel));
...
static void
on_appareance_font_cancel_btn_clicked (GtkWidget * btn, gpointer data)
{
  PreferencesFont *widget;
...
static void
on_appareance_font_btn_clicked (GtkWidget * button, gpointer data)
{
  ...
  gtk_window_set_transient_for (GTK_WINDOW (widgets->window),
				GTK_WINDOW (WMain->window));
  ...
  gtk_font_selection_set_preview_text (GTK_FONT_SELECTION
				       (widgets->font_sel),
				       _
				       ("Pablo was in Paris. Now he lives in Spain"));
  ...
  gtk_signal_connect (GTK_OBJECT (btn), "clicked",
		      GTK_SIGNAL_FUNC
		      (on_appareance_font_cancel_btn_clicked),
		      (gpointer) widgets);
  ...
  gtk_widget_show (btn);
  gtk_signal_connect (GTK_OBJECT (btn), "clicked",
		      GTK_SIGNAL_FUNC (on_appareance_font_ok_btn_clicked),
		      (gpointer) widgets);
...
prefs_add_page_interface (void)
{
   ...
   ** Radio Buttons **
   *******************/
  appareance_toolbar_both =
    gtk_radio_button_new_with_label (NULL, _("Icons and text"));
  gtk_box_pack_start (GTK_BOX (_vbox), appareance_toolbar_both, FALSE, TRUE,
		      0);
  gtk_widget_show (appareance_toolbar_both);
  if (rc->toolbar == GTK_TOOLBAR_BOTH)
    gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON
				  (appareance_toolbar_both), TRUE);

  appareance_toolbar_xpm =
    gtk_radio_button_new_with_label (gtk_radio_button_group
				     (GTK_RADIO_BUTTON
				      (appareance_toolbar_both)),
				     _("Icons only"));
  gtk_box_pack_start (GTK_BOX (_vbox), appareance_toolbar_xpm, FALSE, TRUE,
		      0);
  gtk_widget_show (appareance_toolbar_xpm);
  if (rc->toolbar == GTK_TOOLBAR_ICONS)
    gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON
				  (appareance_toolbar_xpm), TRUE);

  appareance_toolbar_text =
    gtk_radio_button_new_with_label (gtk_radio_button_group
				     (GTK_RADIO_BUTTON
				      (appareance_toolbar_xpm)),
				     _("Text only"));
  gtk_box_pack_start (GTK_BOX (_vbox), appareance_toolbar_text, FALSE, TRUE,
		      0);
  gtk_widget_show (appareance_toolbar_text);
  if (rc->toolbar == GTK_TOOLBAR_TEXT)
    gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON
				  (appareance_toolbar_text), TRUE);

  {
    char *label[2] = { _("Interface"), NULL };

    ctree_interface =
      gtk_ctree_insert_node (GTK_CTREE (sections_ctree), NULL, NULL,
			     label, 4, NULL, NULL, NULL, NULL, FALSE, TRUE);
...
prefs_add_page_fonts (void)
{
  ...
  gtk_table_attach (GTK_TABLE (table), appareance_font_message_body, 1, 2,
  ...
  gtk_entry_set_text (GTK_ENTRY (appareance_font_message_body),
		      rc->font_body);
  ...
  gtk_table_attach (GTK_TABLE (table), appareance_font_unread_message_list,
		    1, 2, 1, 2, (GtkAttachOptions) (GTK_EXPAND | GTK_FILL),
		    (GtkAttachOptions) (0), 0, 0);
  gtk_widget_show (appareance_font_unread_message_list);
  gtk_entry_set_editable (GTK_ENTRY (appareance_font_unread_message_list),
			  FALSE);
  gtk_entry_set_text (GTK_ENTRY (appareance_font_unread_message_list),
		      rc->font_unread);
  ...
  gtk_table_attach (GTK_TABLE (table), appareance_font_read_message_list, 1,
		    2, 2, 3, (GtkAttachOptions) (GTK_EXPAND | GTK_FILL),
		    (GtkAttachOptions) (0), 0, 0);
  gtk_widget_show (appareance_font_read_message_list);
  gtk_entry_set_editable (GTK_ENTRY (appareance_font_read_message_list),
			  FALSE);
  gtk_entry_set_text (GTK_ENTRY (appareance_font_read_message_list),
		      rc->font_read);
  ...
  gtk_signal_connect (GTK_OBJECT (btn), "clicked",
		      ...
		      (gpointer) appareance_font_message_body);
...
prefs_add_page_colors (void)
{
  ...
  gtk_table_attach (GTK_TABLE (table),
		    appareance_colors_reply_original_message, 1, 2, 1, 2,
		    (GtkAttachOptions) (0), (GtkAttachOptions) (0), 0, 0);
  ...
  gtk_table_attach (GTK_TABLE (table), appareance_colors_misc_body, 1, 2, 3,
		    4, (GtkAttachOptions) (0), (GtkAttachOptions) (0), 0, 0);
...
prefs_add_page_advanced (void)
{
   ...
   ***********************************/
  adj =
    (GtkAdjustment *) gtk_adjustment_new (config->timeout, 0, 150, 1, 100, 0);
  ...
  advanced_use_persistent_smtp_connection =
    gtk_check_button_new_with_label (_("Use persistent SMTP connection: "));
  gtk_box_pack_start (GTK_BOX (hbox),
		      advanced_use_persistent_smtp_connection, FALSE, FALSE,
		      0);
  ...
  gtk_tooltips_set_tip (tooltips, advanced_use_persistent_smtp_connection,
			_
			("Keep connection to the SMTP host open. Enables Outgoing messages at any time "),
			NULL);

  /** Persistent SMTP address **/
  advanced_persistent_smtp_address = gtk_entry_new ();
  gtk_box_pack_start (GTK_BOX (hbox), advanced_persistent_smtp_address,
		      TRUE, TRUE, 0);
  gtk_widget_show (advanced_persistent_smtp_address);
  if (config->persistent_smtp_address)
    gtk_entry_set_text (GTK_ENTRY (advanced_persistent_smtp_address),
			config->persistent_smtp_address);
  if (!config->use_persistent_smtp_connection)
    gtk_widget_set_sensitive (advanced_persistent_smtp_address, FALSE);
  ...
  /** Persistent SMTP port **/
  adj =
    (GtkAdjustment *) gtk_adjustment_new (config->persistent_smtp_port, 1,
					  99999, 1, 100, 0);
  advanced_persistent_smtp_port = gtk_spin_button_new (adj, 1, 0);
  gtk_box_pack_start (GTK_BOX (hbox), advanced_persistent_smtp_port, FALSE,
		      FALSE, 0);
  ...
  gtk_signal_connect (GTK_OBJECT (advanced_use_persistent_smtp_connection),
		      "toggled",
		      GTK_SIGNAL_FUNC
		      (on_advanced_use_persistent_smtp_connection_toggled),
		      NULL);
  ...
  gtk_box_pack_start (GTK_BOX (hbox), advanced_addrbook_init, FALSE, FALSE,
		      0);
  ...
  gtk_tooltips_set_tip (tooltips, advanced_use_persistent_smtp_connection,
			_
			("Keep SMTP host connected. This may speed up the sending of "
  ...
  gtk_option_menu_set_history (GTK_OPTION_MENU (advanced_addrbook_init),
			       config->addrbook_init);
				    ...
				    GdkEventButton * event, gpointer data)
{
  ...
  if (s->configure)
    gtk_widget_set_sensitive (plugins_loaded_btn_configure, TRUE);
  else
    gtk_widget_set_sensitive (plugins_loaded_btn_configure, FALSE);
...
on_plugins_load_btn_clicked (void)
{
  ...
  char *buf, *buf2;
  char *file =
    gui_select_file_new (DATADIR "/cronosII/plugins", NULL, "*.so", NULL);
  ...
  if (!c2_file_exists (dir))
      ...
      if (mkdir (dir, 0700) < 0)
	{
	  ...
	  buf =
	    g_strdup_printf (_("Could not create the directory %s: %s"),
			     dir, buf2);
	  c2_free (dir);
  ...
  c2_free (dir);
  dir =
    g_strconcat (getenv ("HOME"), ROOT, "/Plugins/", g_basename (file), NULL);
  if (c2_dynamic_module_load (file, &config->module_head))
    fd_bin_cp (file, dir);
  on_plugins_refresh ();
}
...
on_plugins_unload_btn_clicked (void)
{
  ...
  if (!GTK_CLIST (plugins_loaded_clist)->selection)
    return;

  gtk_clist_get_text (GTK_CLIST (plugins_loaded_clist),
		      (int) GTK_CLIST (plugins_loaded_clist)->selection->data,
		      0, &name);
  if (!name)
    return;
  ...
  if (widget)
    {
      ...
      gtk_signal_connect (GTK_OBJECT (widget), "destroy",
			  GTK_SIGNAL_FUNC
			  (on_on_plugins_configure_btn_clicked_destroy),
			  NULL);
    }
}

static void
prefs_add_page_plugins (void)
{
  ...
  pixmap =
    gnome_stock_pixmap_widget_at_size (window, GNOME_STOCK_PIXMAP_FORWARD,
				       16, 16);
  ...
  gtk_container_set_border_width (GTK_CONTAINER (hbox2), 2);
  pixmap =
    gnome_stock_pixmap_widget_at_size (window, GNOME_STOCK_PIXMAP_BACK,
				       16, 16);
  ...
  gtk_container_set_border_width (GTK_CONTAINER (hbox2), 2);
  pixmap =
    gnome_stock_pixmap_widget_at_size (window, GNOME_STOCK_PIXMAP_INDEX,
				       16, 16);
  label = gtk_label_new (_("Configure"));
  gtk_box_pack_start (GTK_BOX (hbox2), pixmap, FALSE, FALSE, 3);
  gtk_box_pack_start (GTK_BOX (hbox2), label, FALSE, FALSE, 3);
  gtk_container_add (GTK_CONTAINER (plugins_loaded_btn_configure), hbox2);
  gtk_box_pack_start (GTK_BOX (vbox2), plugins_loaded_btn_configure, FALSE,
		      FALSE, 0);
  ...
  gtk_signal_connect (GTK_OBJECT (plugins_loaded_btn_configure), "clicked",
		      GTK_SIGNAL_FUNC (on_plugins_configure_btn_clicked),
		      NULL);
  ...
  gtk_table_attach (GTK_TABLE (table), plugins_label_description, 1, 2, 4,
		    5, (GtkAttachOptions) (GTK_EXPAND | GTK_FILL),
