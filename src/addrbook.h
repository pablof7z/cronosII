...
#define C2VCARDPHONE(obj)			((C2VCardPhone*)obj)
	...
	void c2_address_book_init (void);

	  gint c2_address_book_get_num_of_cards (void);

	const C2VCard *c2_address_book_card_get (guint offset);

	int c2_address_book_card_add (const C2VCard * card);

	void c2_address_book_card_remove (const C2VCard * card);

	void c2_address_book_flush (void);

	void c2_address_book_card_free (C2VCard * card);

	int
		c2_address_book_search (GList * search_list,
					const char *pattern, guint offset);

	GList *c2_address_book_search_all (const char *pattern);

	  gboolean c2_address_book_check_name_exists (const gchar * name);

	GList *c2_address_book_get_available_groups (void);

	C2VCard *c2_address_book_get_card_from_str (const char *string);
